...
app.post('/checktokenold',function(req,res){
	var tokenString = req.body.token;
	db.get(tokenString, function(err, data){
			if(!err) {
				console.log("Found document : " + JSON.stringify(data));
				var date = new Date(5501424115*1000);
				
				if(moment().toDate()> date){
					res.contentType('application/json');
					res.send({ok : false,
							msg : "Please re-Authenticate. Expired Token"});
			    ...
			    if(req.body.type === 'facebook'){
			        var toBeVerified = req.body.token;
	                var appToken = '1484283014936731|uuQr6tL3RAnVXJwfhO5E6dll_zg';
		            var facebookURL = 'https://graph.facebook.com/debug_token?%20input_token='+toBeVerified+'&access_token='+appToken;
		            request(facebookURL, function(error, response, body){
	            	var isValid = JSON.parse(body).error_description === undefined;
	            	if(isValid){
	            	   db.insert({_id: toBeVerified,
      		            duration : JSON.parse(body).exp},toBeVerified, function(err, data){
					if (!err)
					{
					...
					else
					{
	            		...
	            		res.send("faceook invalid "+JSON.parse(body).error_description);
	            	}
                        ...
                        var url  = 'https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=' + googleToken;
                         ...
                         request(url, function(error, response, body){
                          ...
                          //a token should only be valid if the JSON object does not have
                           //an error field
                        var isValid = JSON.parse(body).error_description === undefined||"Invalid Value";
                        
                    ;
                    	if(isValid){
                    	     db.insert({_id: googleToken,
      		            duration : JSON.parse(body).exp},googleToken, function(err, data){
					if (!err)
					{
						console.log("Added new document");
						res.contentType('application/json'); //res.contentType and res.send is added inside every block as code returns immediately
					...
					else
					{
						...
						res.send({error : err});
                        		...
                        		res.send("invalid"+JSON.parse(body).error_description);
                        	}
...
app.post('/checktokennew',function(req,res){
	var tokenString = req.body.token;
	db.get(tokenString, function(err, data){
			if(!err) {
				console.log("Found document : " + JSON.stringify(data));
...
app.post('/checktoken',function(req,res){
	var tokenString = req.body.token;
	db.find({selector:{token:tokenString}}, function(er, result) {
  if (!er) {
      if(result.docs.length>0){
	console.log("Found document : " + result.docs[0]._id);
				var date = new Date(result.docs[0].duration*1000);
				if(moment().toDate()> date){
					res.contentType('application/json');
					res.send({ok : false,
							msg : "Please re-Authenticate. Expired Token"});
        ...
        if(req.body.type === 'facebook'){
            ...
            var isValid = JSON.parse(body).error_description === undefined;
            if(isValid){
               db.insert({token: toBeVerified,
                  duration : JSON.parse(body).exp}, function(err, data){
            if (!err)
            {
                console.log("Added new document");
                res.contentType('application/json'); //res.contentType and res.send is added inside every block as code returns immediately
            ...
            else
            {
                ...
                var url  = 'https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=' + googleToken;
                 ...
                 request(url, function(error, response, body){
                  ...
                  //a token should only be valid if the JSON object does not have
                   //an error field
               // var isNotValid = JSON.parse(body).error_description === undefined||"Invalid Value";
               var isNotValid = JSON.parse(body).error_description === "Invalid Value";
                
            ;
                if(isNotValid){

                    res.send({"error":JSON.parse(body).error_description});
                        
                    } else {
                        console.log("Exppp: "+JSON.parse(body).exp);
                        db.insert({token: googleToken,
                     duration : JSON.parse(body).exp}, function(err, data){
               if (!err)
               {
                   console.log("Added new document");
...
app.get('/testget',function(req,res){
	db.find({selector:{duration:req.body.duration}}, function(er, result) {
  if (er) {
  	throw er
  }
